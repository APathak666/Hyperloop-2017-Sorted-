...
#define STEPPER 19  //stepper_motor1 - pins??
...
void CANReceive3(void *args){
  for (;;){
    ...
    taskDISABLE_INTERRUPTS();
    analogWriteFrequency(STEPPER, (uint16_t) *(msgRec[3].buf)); 
    taskENABLE_INTERRUPTS();    
  }
}
...
void CANReceive8(void *args){
  for (;;){
    ...
    */
    ...
    analogWriteFrequency(STEPPER,STEPPERFREQ); 
    delayMicroseconds((1000000 * numPulses)/STEPPERFREQ);
    digitalWriteFast(STEPPER, LOW);
    taskENABLE_INTERRUPTS();
  }
}
...
void setup(){
  ...
  canListener.attachMBHandler(7);
  canListener.attachMBHandler(8);
  canListener.attachMBHandler(9);
  canListener.attachMBHandler(10);
  canListener.attachMBHandler(11);
